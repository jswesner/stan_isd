...
reps = 4 # replicates per predictor value
n_ind = 300 # number of individuals to simulate
intercept = -1.2
var_int_sd = 0.01 # sd of the varying intercept
beta = -0.05 # regression slope
n_groups = 1
xmin = 1
xmax = 1000
...
data_grid = tibble(predictor = -seq(-1, 1, length.out = 5)) %>% 
...
reg_fits = replicate(update(reg_fit, newdata = make_regression_data(), chains = 4, iter = 2000), 
                     n = 1000) 

# saveRDS(reg_fits, file = "models/fig3_mods.rds")